class Game {
  ...
  play(){
    ...
    if(allPlayers !== undefined){
      ...
      for(var plr in allPlayers){
       ...
       // console.log(index, player.index)
        ...
        if(frameCount%50 ===0){
          hurdlesy=Math.round(random(displayHeight*3,-displayHeight*3.5));
          hurdlesx=Math.round(random(350,1300));
          
          barricading=createSprite(hurdlesx,hurdlesy);
          barricading.addImage("barricadingImage", barricadingImage);
          barricading.scale=0.2;
         
        hurdlesGroup.add(barricading);
        }
    ...
    if (hurdlesGroup.isTouching(cars)){
   ...
   // for(var i=0;i<20;i++){
    ...
    if(player.distance > 4500){
     ...
     lap2=createSprite(700,500);
      player.rank+=1;
      textSize(30);
         text(player.name+" "+player.rank,100,y+50);
